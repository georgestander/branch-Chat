...
Branch Chat uses Cloudflare bindings locally. Chat authentication comes from the local Codex app-server, not an API key.

## Deployment Profiles

| Profile | Intended use | Minimum runtime vars | Notes |
| --- | --- | --- | --- |
| Local ChatGPT | Local development | None after `codex login` | The bridge listens on loopback only and derives the stable local user from the ChatGPT email. |

## Runtime Variables

| Name | Required | Purpose | Default/Fallback |
| --- | --- | --- | --- |
| `CODEX_BRIDGE_URL` | No | Worker-side URL for the local Codex bridge | `http://127.0.0.1:43991` |
