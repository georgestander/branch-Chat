...
export function normalizeAccountState(
  ...
  const normalizedOwnerId =
    typeof record.ownerId === "string" && record.ownerId.trim().length > 0
      ? record.ownerId.trim()
      : ownerId;
  ...
  const byok = normalizeByokCredential(record.byok ?? null, now);
  const composerPreference = normalizeComposerPreference(
    record.composerPreference ?? null,
    now,
  );
...
export class AccountDO implements DurableObject {
  ...
  private async getState(ownerId: string): Promise<AccountState> {
    ...
    const stored = (await this.state.storage.get<unknown>(STORAGE_KEY)) ?? null;
    const normalized = normalizeAccountState(stored, ownerId);
...
export class AccountClient {
  ...
  constructor(stub: DurableObjectStub, ownerId: string) {
    this.stub = stub;
